...
calculated_metrics AS (
    SELECT 
        ...
        ctm_region,
        ctm_country,
        first_order_clean_date,
        last_order_clean_date,
        total_orders,
        total_revenue,
        avg_order_value,
        total_quantity,
        ...
        customer_segment,
        CASE 
            WHEN last_order_clean_date = CAST({{ var("date_default") }} AS DATE) THEN 9999
            ELSE DATEDIFF(CAST('{{ var("current_business_date") }}' AS DATE), last_order_clean_date)
        END AS recency_days,
        CASE 
            WHEN first_order_clean_date = CAST({{ var("date_default") }} AS DATE) THEN 0
            ELSE DATEDIFF(last_order_clean_date, first_order_clean_date) 
        END AS customer_tenure_days,
        CAST(COALESCE(total_revenue / NULLIF(total_orders, 0), 0) AS DECIMAL(15,2)) AS calculated_aov,
        CAST(
            CASE 
...
SELECT 
    ...
    calculated_aov AS avg_order_value,
    ...
    CASE 
        WHEN recency_days > 90 THEN TRUE 
        ELSE FALSE 
    END AS is_churned,
    customer_health_score,
    CAST(calculated_aov * (1.00 - customer_health_score) AS DECIMAL(15,2)) AS revenue_at_risk,
