...
customer_base AS (
  SELECT
    ...
    SUM(
      CASE
        WHEN DATEDIFF(
          ...
          ord_order_date
        ) <= 365 THEN order_value
        ELSE 0
      END
    ) AS revenue_last_365d,
    ...
    AVG(
      order_discount / NULLIF(
        order_value + order_discount,
        0
      )
    ) AS avg_discount_pct,
  ...
  FROM
    (
      SELECT
        ...
        DATEDIFF(
          ord_order_date,
          LAG(ord_order_date) OVER (
            PARTITION BY sk_customer
            ORDER BY
              ord_order_date
          )
...
FROM
  customer_base cb
  LEFT JOIN customer_behavior beh
  ON cb.sk_customer = beh.sk_customer
