...
WITH source AS (
  ...
  SELECT
    ...
    COALESCE(CAST(order_date AS DATE), CAST({{ var('date_default') }} AS DATE)) AS ord_order_date,
    COALESCE(CAST(required_date AS DATE), CAST({{ var('date_default') }} AS DATE)) AS ord_required_date,
    COALESCE(CAST(shipped_date AS DATE), CAST({{ var('date_default') }} AS DATE)) AS ord_shipped_date,
